...
namespace AprajitaRetails.Ops.Utility
{
    ...
    public class CashBookManagerExporter
    {
        ...
        public List<CashBook> GetMontlyCashBook(AprajitaRetailsContext db, DateTime date)
        {
            ...
            var eCPay = db.CashPayments.Where(c => c.PaymentDate.Month == date.Month && c.PaymentDate.Year == date.Year).OrderBy(c => c.PaymentDate);//ok
            var ePay = db.Payments.Where(c => c.PayMode == PaymentModes.Cash && c.PayDate.Month == date.Month && c.PayDate.Year == date.Year).OrderBy(c => c.PayDate);
            var eStPay = db.StaffAdvancePayments.Include(e => e.Employee).Where(c => c.PayMode == PayModes.Cash && c.PaymentDate.Month == date.Month && c.PaymentDate.Year == date.Year).OrderBy(c => c.PaymentDate);
            var eSal = db.SalaryPayments.Include(e => e.Employee).Where(c => c.PayMode == PayModes.Cash && c.PaymentDate.Month == date.Month && c.PaymentDate.Year == date.Year).OrderBy(c => c.PaymentDate);
            var eexp = db.Expenses.Where(c => c.PayMode == PaymentModes.Cash && c.ExpDate.Month == date.Month && c.ExpDate.Year == date.Year).OrderBy(c => c.ExpDate);
